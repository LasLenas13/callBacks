...
const logpara = (para1, para2) => console.log(`the first parameter is ${para1}. The second parameter is ${para2}.`)
...
function legalName(fullName) {
    console.log(`Hello, my full name is ${fullName}.`);
  }
  
  greeting('Johnny', 'Rocket', legalName);
...
function pricesPlusTax(prices, callback){
    ...
    for(let i = 0; i < prices.length; i++){
        const withTax = prices[i] * 1.2;
        totalCost.push(withTax)
    }
    callback(totalCost)
}
...
function badTax(totalCost) {
    console.log(totalCost);
  }

pricesPlusTax(prices, badTax);
